...
cat deletejudul3.txt | awk 'FNR==0{print ""}{print}' | while read line; do
    ...
    echo "cat b.html | sed -e 's/judulstrip1/$(cat deletejudul1strip.txt)/g' -e 's/gambar1/$(cat deletegambar1.txt)/g' -e 's/gambar2/$(cat deletegambar2.txt)/g' -e 's/judulgambar/$(cat deletejudulgambar.txt)/g' -e 's/judul1/$(cat deletejudul1.txt)/g' >> deletegallery.html" | bash -
done
...
sed -n 1p deletekeyword1.txt | sed 's|.*|\\<a href=\\"http:\\/\\/www\\.bedsiana\\.com\\/image\\/title2\\" target=\\"_blank\\" \\>&\\<\\/a\\>|' > deletekeyword.txt
...
echo "cat deletejudul31.txt | sed 's|.*|\\\<a href=\\\"http:\\\/\\\/www\\\.bedsiana\\\.com\\\/$(cat deletejudul1strip.txt)\\\/$(sed -n 1p deletejudul3strip.txt)\\\" target=\\\"_blank\\\"\\>&\\\<\\\/a\\\>|' > deletejudul32.txt" | bash -
...
echo "exiftool -Title=\"$(cat deletejudul2.txt) - $(cat deletejudul1.txt)\" -Author=\"$(cat deleteauthor1.txt)\" -Description=\"The picture/image of $(cat deletejudul2.txt) - $(cat deletejudul1.txt)\" -Keyword=\"$(cat deletejudul2.txt), $(cat deletejudul1.txt), $(cat deletetags2.txt)\" -Copyright=\"Source http://www.bedsiana.com/image/title2\" Done/title1/title2" | bash -
# sitemap
...
# mysql
echo "$(cat deletesql.txt)" | awk 1 ORS='' | sed '$a\' >> Report/mysql.csv
