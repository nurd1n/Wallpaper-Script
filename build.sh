...
echo "awk 'FNR==1{print \"<div id='gallery-1'><center><h2>$(cat deletejudul2.txt)</h2></center>\"}{print}' deletegallery2.html | sed '$ a </div>' | tr '\n' ' ' > deletegallery.html" | bash -
# spin article
...
#upload youtube
echo "wget -qO- --header=\"Accept: text/html\" --user-agent=\"Googlebot-Image/1.0\" \"http://www.bing.com/videos/search?scope=video&pq=$(echo title1 | sed 's/+2016//g')&sc=1-30&sp=-1&sk=&q=$(echo title1 | sed 's/+2016//g')&qft=+filterui:msite-youtube.com\" | grep -o '<a href=['\"'\"'\"][^\"'\"'\"']*['\"'\"'\"]' | grep 'watch?v' | sed -e 's/^<a href=[\"'\"'\"']//' -e 's/[\"'\"'\"']$//' -e 's/https:\\/\\/www.youtube.com\\/watch?v=//g' | shuf -n 1 > deleteyoutube.txt" | bash -
echo "<br/><div class='video-container'><iframe type=\"text/html\" width=\"560\" height=\"315\" src=\"https://www.youtube-nocookie.com/embed/$(cat deleteyoutube.txt)?autohide=1&rel=0&controls=0&modestbranding=1&disablekb=1&theme=light&enablejsapi=1\" frameborder=\"0\"></iframe></div>" >> deletepar3.txt
# shuffle templates
...
echo "$(shuf -i 2014-2015 -n 1)-$(shuf -i 1-12 -n 1 | sed 's/.*/0&/' | tail -c 3)-$(shuf -i 1-28 -n 1 | sed 's/.*/0&/' | tail -c 3)" > deletetanggal.txt
...
echo "exiftool -Title=\"$(cat deletejudul2.txt) - $(cat deletejudul1.txt)\" -Author=\"$(cat deleteauthor1.txt)\" -Description=\"The picture/image of $(cat deletejudul2.txt) - $(cat deletejudul1.txt)\" -Keyword=\"$(cat deletejudul2.txt), $(cat deletejudul1.txt)\" -Copyright=\"Source http://www.domain.ekstension/wp-content/uploads/title2\" done/title1/title2" | bash -
# sql file 1
...
# upload image
cd /home/www/domain
echo "wp media import /home/wallpaperdomainekstension/attachment/done/title1/title2 --post_id=$(cat /home/wallpaperdomainekstension/attachment/id/title1.txt) --porcelain --allow-root > /home/wallpaperdomainekstension/attachment/deleteid.txt" | bash -
# sql file 2
cd /home/wallpaperdomainekstension/attachment
...
# mysql query
cd /home/www/domain
wp db query --allow-root < /home/wallpaperdomainekstension/attachment/deletemysql.sql
# search & replace
echo "UPDATE wp_posts SET post_content = replace(post_content,\"http://www.domain.ekstension/$(cat /home/wallpaperdomainekstension/attachment/deletejudul1strip.txt)/\",\"http://www.domain.ekstension/$(cat /home/wallpaperdomainekstension/attachment/id/title1.txt)/$(cat /home/wallpaperdomainekstension/attachment/deletejudul1strip.txt)/\")" > /home/wallpaperdomainekstension/attachment/deletemysql.sql
wp db query --allow-root < /home/wallpaperdomainekstension/attachment/deletemysql.sql
cd /home/wallpaperdomainekstension/attachment
# sitemap
echo "cat data/sitemap | sed -e 's|judul1|$(< \deletejudul1strip.txt)|g' -e 's|judul2|$(< \deletejudul2strip.txt)|g' -e 's|tanggal|$(< \deletetanggal.txt)|g' -e 's|waktu|$(< \deletewaktu.txt)|g' -e 's|postid|$(cat /home/wallpaperdomainekstension/attachment/id/title1.txt)|g' >> sitemap-attachment.xml" | bash -
