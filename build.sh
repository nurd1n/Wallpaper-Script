...
if [ $(find asin -type f | wc -l) -lt "20" ]; then
  ...
  echo "for a in *.jpg; do mv -- \$a asin/$(< \deletetitle2.txt)-\${a%.jpg}-$(< \deletenumber.txt).jpg; done" | bash -
  echo "for a in *.JPG; do mv -- \$a asin/$(< \deletetitle2.txt)-\${a%.JPG}-$(< \deletenumber.txt).jpg; done" | bash -
  echo "for a in *.png; do mv -- \$a asin/$(< \deletetitle2.txt)-\${a%.png}-$(< \deletenumber.txt).jpg; done" | bash -
  echo "for a in *.jpeg; do mv -- \$a asin/$(< \deletetitle2.txt)-\${a%.jpeg}-$(< \deletenumber.txt).jpg; done" | bash -
  echo "for a in *.jpg; do cp -- \$a asin/$(< \deletetitle2.txt)-\${a%.jpg}-$(< \deletenumber.txt).jpg; done" | bash -
  echo "for a in *.JPG; do cp -- \$a asin/$(< \deletetitle2.txt)-\${a%.JPG}-$(< \deletenumber.txt).jpg; done" | bash -
  echo "for a in *.png; do cp -- \$a asin/$(< \deletetitle2.txt)-\${a%.png}-$(< \deletenumber.txt).jpg; done" | bash -
  echo "for a in *.jpeg; do cp -- \$a asin/$(< \deletetitle2.txt)-\${a%.jpeg}-$(< \deletenumber.txt).jpg; done" | bash -
  cd asin
  for f in *.jpg; do echo "mv -- $f \$(echo $f | sed 's/^-*//')" | bash -; done
  cd -
  find ./asin/* -wholename "*.jpg" -size -15k -delete
  find ./asin/* -not -empty -type f -printf '%s\n' | sort -rn | uniq -d | xargs -I{} -n1 find ./asin/* -type f -size {}c -print0 | xargs -0 md5sum | sort | uniq -w32 --all-repeated=separate | cut -c 37- | sed -n '1!p' | xargs shred -v -n 25 -u -z
