...
for f in image/asin/*.jpg; do mv -- $f $(echo $f | sed -e 's/.jpg//g' -e 's/image\/asin\///g' -e 's/\./-/g' -e 's/_/-/g' -e 's/[+_,;: (){}]/-/g' -e 's/---/-/' -e 's/---/-/' -e 's/--/-/' -e 's/--/-/' -e 's/--/-/' | cut -c -180 | tr '[A-Z]' '[a-z]' | sed 's/.*/image\/asin\/&.jpg/') ; done
rm -r -f asin
